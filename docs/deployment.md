...
Firebase Cloud Messaging을 활성화할 때만 다음 공개 설정을 추가한다.
...
`SUPABASE_SERVICE_ROLE_KEY`, Firebase 서비스 계정 JSON, Google OAuth Client Secret은
브라우저에 포함되므로 Vercel 빌드 환경변수에 넣지 않는다. 서버용 값은 해당
Supabase Edge Function의 secret으로만 관리한다.

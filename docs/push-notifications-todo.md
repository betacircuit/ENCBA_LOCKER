...
원격 푸시의 앱·DB·발송 코드가 작업 중이며, 외부 서비스 설정과 실제 배포 검증은 아직
끝나지 않았다.
...
- `push_subscriptions`와 `push_deliveries` 마이그레이션이 구독 소유권과 중복 발송 방지를
  정의한다.
- `supabase/functions/send-push`가 긴급 공지와 출결 마감 3시간 전 후보를 FCM HTTP v1으로
  발송한다.
...
3. Firebase 백그라운드 메시지를 받을 `web/firebase-messaging-sw.js`를 작성하고 Flutter
   부트스트랩 전에 등록한다. 이 파일이 없으면 탭이 닫힌 웹 푸시를 완료할 수 없다.
